...
class PostTestCase(TestCase):
    ...
    def setUp(self):
        ...
        tag = Tag(tag_id=1, tag_name="happy")
        post = Post(
            title="Spotted a UFO",
    ...
    def test_if_new_tag_added(self):
        ...
        data = {"tag_id": 1, "tag_name": "sad"}
        with app.test_client() as client:
            resp = client.post("/tags/add-tag")
            html = resp.get_data(as_text=True, data=data, follow_redirects=True)

            self.assertEqual(resp.status_code, 200)
            self.assertIn(
                f'<a href="/tags/1" class="list-group-item list-group-item-action">sad</a>',
                html,
            )

    def test_if_edited_tag_added(self):
        """Check if tag was updated by edited info from form data"""
        data = {"tag_id": 1, "tag_name": "angry"}
        with app.test_client() as client:
            resp = client.post(f"/tags/1/edit")
            html = resp.get_data(as_text=True, data=data, follow_redirects=True)

            self.assertEqual(resp.status_code, 200)
            self.assertIn(
                f'<a href="/tags/1" class="list-group-item list-group-item-action">angry</a>',
                html,
            )
